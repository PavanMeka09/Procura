...
export const VENDOR_IDS = { apex: '4bce3f2e-ef21-4c93-b2bb-4e8a2a2f5c01', northstar: '5b7d2c42-f11e-4f0f-a3c3-8b8c4a1d9b02', vertex: '6c9e4d53-a22f-4a1f-b4d4-9c9d5b2e0c03' } as const;
...
export const seededVendors = (requestId: string): Vendor[] => [
  { id: VENDOR_IDS.apex, slug: 'vendor-a', name: 'Apex Devices', category: 'Business hardware', approved: true, reliabilityScore: 0.84, contact: 'sales@apex.example', behavior: { initial: offer(requestId, VENDOR_IDS.apex, 1, 60000, 20, 24, 50, '50% advance, balance before dispatch'), rounds: [offer(requestId, VENDOR_IDS.apex, 2, 57000, 20, 24, 30, '30% advance, balance on dispatch'), offer(requestId, VENDOR_IDS.apex, 3, 56500, 20, 24, 20, '20% advance, balance on delivery')], failure: 'DELAY_ONCE' } },
  { id: VENDOR_IDS.northstar, slug: 'vendor-b', name: 'Northstar IT', category: 'Business hardware', approved: true, reliabilityScore: 0.91, contact: 'rfq@northstar.example', behavior: { initial: offer(requestId, VENDOR_IDS.northstar, 1, 62000, 14, 36, 20, '20% advance, balance on delivery'), rounds: [offer(requestId, VENDOR_IDS.northstar, 2, 57500, 14, 36, 20, '20% advance, balance on delivery'), offer(requestId, VENDOR_IDS.northstar, 3, 56000, 14, 36, 20, '20% advance, balance on delivery')], failure: 'MALFORMED_ONCE' } },
  { id: VENDOR_IDS.vertex, slug: 'vendor-c', name: 'Vertex Systems', category: 'Business hardware', approved: true, reliabilityScore: 0.96, contact: 'commercial@vertex.example', behavior: { initial: offer(requestId, VENDOR_IDS.vertex, 1, 58000, 21, 24, 20, '20% advance, balance on delivery'), rounds: [offer(requestId, VENDOR_IDS.vertex, 2, 56000, 21, 24, 20, '20% advance, balance on delivery'), offer(requestId, VENDOR_IDS.vertex, 3, 55500, 21, 24, 20, '20% advance, balance on delivery')], failure: 'TEMPORARY_FAILURE_ONCE' } },
];
