...
import { persistModelRun, persistSession, persistToolExecution, persistVendor } from '../db/repository';
...
async function getOfferWithRecovery(session: NegotiationSession, vendor: Vendor, round: number): Promise<Offer | null> {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      let response;
      try { response = getVendorResponse(vendor, round, session.requestId, session.originalRequest, failureConsumed); toolSucceeded = !('failure' in response); }
      finally { recordTool(session, { toolName: 'vendor.get_response', durationMs: Date.now() - toolStarted, success: toolSucceeded, retryCount: attempt, input: { vendorId: vendor.id, round } }); }
...
async function negotiateVendor(session: NegotiationSession, vendor: Vendor, adapters: ModelAdapters) {
...
export async function resumeSession(sessionId: string, adapters: ModelAdapters = defaultAdapters) {
  ...
  if (!session || !session.humanReview || session.humanReview.status !== 'APPROVED' || !session.pendingAction) return session;
  const action = session.pendingAction;
  const offer = session.offers.find((item) => item.id === ('offerId' in action ? action.offerId : session.offers.at(-1)?.id));
  if (!offer) return session;
  let critic;
  try { critic = await adapters.critic.critique(session, offer, action); recordModelRuns(session, [critic.run], session.currentRound); } catch { session.humanReview.status = 'PENDING'; session.humanReview.resolvedAt = undefined; session.currentState = 'HUMAN_REVIEW'; session.stopReason = 'Independent critic remains unavailable; approval was not executed.'; emit(session, 'HUMAN_REVIEW_REQUIRED', 'HUMAN_REVIEW', session.stopReason, { reviewId: session.humanReview.id }); return session; }
